  ...
  async function sendWeeklyFollowUp(msg: string) {
    ...
    try {
      ...
      const weeklyCtx = [
        ...
        `⚠️ CURRENT AGE: ${ctx.currentAge} | Year: ${new Date().getFullYear()} — For timing questions: scan antardasha sub-periods in ~3-year blocks from NOW. Never predict marriage beyond age 45. Always include a window within the next 5 years.`,
  ...
  async function sendFollowUp(msg: string) {
    ...
    try {
      ...
      const chartSummary = [
        ...
        `⚠️ CURRENT AGE: ${ctx.currentAge} | Current year: ${new Date().getFullYear()} — For ANY timing question: first identify current Maha Dasha, then scan antardasha sub-periods in ~3-year blocks from NOW to find the nearest favorable window. NEVER predict marriage beyond age 45. Must always include at least one window within the next 5 years.`,
